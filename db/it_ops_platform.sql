...
-- ----------------------------
-- Table structure for it_ticket_category
-- ----------------------------
DROP TABLE IF EXISTS `it_ticket_category`;
CREATE TABLE `it_ticket_category`  (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '工单分类ID，主键',
  `parent_id` bigint(20) NULL DEFAULT NULL COMMENT '父级分类ID，用于支持多级分类，一级分类为空',
  `name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '分类名称，例如电脑故障、网络故障、打印机故障',
  `code` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '分类编码，例如 computer、network、printer',
  `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '分类说明',
  `default_priority` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '默认优先级：low低，normal普通，high高，urgent紧急',
  `default_group_id` bigint(20) NULL DEFAULT NULL COMMENT '默认运维组ID，用于自动分配',
  `assignment_strategy` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '默认分配策略：least_workload最少工单，fixed_user固定人员',
  `fixed_assignee_id` bigint(20) NULL DEFAULT NULL COMMENT '固定分配人员ID，当 assignment_strategy=fixed_user 时使用',
  `require_asset` tinyint(4) NOT NULL DEFAULT 0 COMMENT '是否要求提交工单时必须关联资产：1是，0否',
  `sort_order` int(11) NOT NULL DEFAULT 0 COMMENT '排序值，越小越靠前',
  `status` tinyint(4) NOT NULL DEFAULT 1 COMMENT '状态：1启用，0停用',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `uk_ticket_category_code`(`code`) USING BTREE,
  INDEX `idx_ticket_category_parent`(`parent_id`) USING BTREE,
  INDEX `idx_ticket_category_status`(`status`) USING BTREE,
  INDEX `idx_ticket_category_sort`(`sort_order`) USING BTREE,
  INDEX `idx_ticket_category_default_group`(`default_group_id`) USING BTREE,
  INDEX `idx_ticket_category_fixed_assignee`(`fixed_assignee_id`) USING BTREE,
  CONSTRAINT `fk_ticket_category_parent` FOREIGN KEY (`parent_id`) REFERENCES `it_ticket_category` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB AUTO_INCREMENT = 8 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '工单分类表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of it_ticket_category
-- ----------------------------
INSERT INTO `it_ticket_category` VALUES (1, NULL, '电脑故障', 'computer', '电脑无法开机、蓝屏、卡顿、外设异常等问题', 'normal', NULL, 'least_workload', NULL, 1, 1, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `it_ticket_category` VALUES (2, NULL, '网络故障', 'network', '无法上网、网络慢、IP冲突、无线网络异常等问题', 'high', NULL, 'least_workload', NULL, 0, 2, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `it_ticket_category` VALUES (3, NULL, '打印机故障', 'printer', '无法打印、卡纸、缺墨、驱动异常等问题', 'normal', NULL, 'least_workload', NULL, 1, 3, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `it_ticket_category` VALUES (4, NULL, '账号权限', 'account', 'OA、ERP、邮箱、VPN等账号权限问题', 'normal', NULL, 'least_workload', NULL, 0, 4, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `it_ticket_category` VALUES (5, NULL, '软件问题', 'software', '办公软件、业务系统、客户端程序异常', 'normal', NULL, 'least_workload', NULL, 0, 5, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `it_ticket_category` VALUES (6, NULL, '服务器故障', 'server', '服务器、数据库、中间件、定时任务异常', 'high', NULL, 'least_workload', NULL, 1, 6, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `it_ticket_category` VALUES (7, NULL, '其他问题', 'other', '无法归类的其他IT问题', 'low', NULL, NULL, NULL, 0, 99, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
...
CREATE TABLE `it_sla_rule`  (
  ...
  `ticket_category` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '兼容字段：旧工单类型字符串；新逻辑使用category_id',
  `category_id` bigint(20) NULL DEFAULT NULL COMMENT '工单分类ID，关联it_ticket_category表；NULL表示通用规则',
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `idx_sla_rule_match`(`category_id`, `priority`, `enabled`) USING BTREE,
  INDEX `idx_sla_rule_legacy_category`(`ticket_category`) USING BTREE,
  CONSTRAINT `fk_sla_rule_category` FOREIGN KEY (`category_id`) REFERENCES `it_ticket_category` (`id`) ON DELETE SET NULL ON UPDATE RESTRICT
...
-- ----------------------------
INSERT INTO `it_sla_rule` VALUES (1, '紧急工单通用SLA', NULL, NULL, 'urgent', 10, 120, 1, 10, '2026-06-24 00:00:00', '2026-06-24 00:00:00');
INSERT INTO `it_sla_rule` VALUES (2, '高优先级通用SLA', NULL, NULL, 'high', 30, 240, 1, 20, '2026-06-24 00:00:00', '2026-06-24 00:00:00');
INSERT INTO `it_sla_rule` VALUES (3, '普通优先级通用SLA', NULL, NULL, 'medium', 60, 480, 1, 30, '2026-06-24 00:00:00', '2026-06-24 00:00:00');
INSERT INTO `it_sla_rule` VALUES (4, '低优先级通用SLA', NULL, NULL, 'low', 240, 2880, 1, 40, '2026-06-24 00:00:00', '2026-06-24 00:00:00');
INSERT INTO `it_sla_rule` VALUES (5, '网络故障紧急SLA', NULL, 2, 'urgent', 5, 60, 1, 1, '2026-06-24 00:00:00', '2026-06-24 00:00:00');
...
CREATE TABLE `it_ticket`  (
  ...
  `category_id` bigint(20) NOT NULL COMMENT '工单分类ID，关联it_ticket_category表',
  ...
  CONSTRAINT `fk_ticket_category` FOREIGN KEY (`category_id`) REFERENCES `it_ticket_category` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
...
-- ----------------------------
INSERT INTO `it_ticket` VALUES (1, 'TK202606210001', '财务电脑无法开机', '按下电源键后电脑无反应，显示器无信号。', 1, 'high', 'completed', 3, 2, 1, '重新插拔内存并清理主板灰尘后恢复正常。', '2026-06-21 09:10:00', 1, 'manual', '2026-06-21 09:20:00', '2026-06-21 09:20:00', '2026-06-21 09:30:00', '2026-06-21 10:10:00', '2026-06-21 09:40:00', '2026-06-21 13:10:00', '2026-06-21 09:20:00', '2026-06-21 10:10:00', 0, 0, '2026-06-22 19:39:11');
INSERT INTO `it_ticket` VALUES (2, 'TK202606210002', '运营部打印机卡纸', '打印合同时频繁卡纸，影响正常办公。', 3, 'normal', 'completed', 4, 2, 2, '清理纸道并更换搓纸轮后恢复正常。', '2026-06-21 10:00:00', 1, 'manual', '2026-06-21 10:15:00', '2026-06-21 10:15:00', '2026-06-21 10:30:00', '2026-06-21 11:00:00', '2026-06-21 11:00:00', '2026-06-21 18:00:00', '2026-06-21 10:15:00', '2026-06-21 11:00:00', 0, 0, '2026-06-22 19:39:11');
INSERT INTO `it_ticket` VALUES (3, 'TK202606210003', '部分电脑访问内网系统较慢', '多个用户反馈访问ERP系统时页面加载缓慢。', 2, 'urgent', 'completed', 3, 2, 3, '排查发现交换机端口异常，调整端口并重启相关网络设备后恢复。', '2026-06-21 11:20:00', 1, 'manual', '2026-06-21 11:25:00', '2026-06-21 11:25:00', '2026-06-21 11:30:00', '2026-06-21 12:10:00', '2026-06-21 11:25:00', '2026-06-21 12:20:00', '2026-06-21 11:25:00', '2026-06-21 12:10:00', 0, 0, '2026-06-22 19:39:11');
INSERT INTO `it_ticket` VALUES (4, 'TK202606210004', '办公软件无法正常启动', '打开Excel后提示组件加载失败。', 5, 'normal', 'pending_accept', 4, NULL, 1, NULL, '2026-06-21 14:00:00', NULL, NULL, NULL, NULL, NULL, NULL, '2026-06-21 15:00:00', '2026-06-21 22:00:00', NULL, NULL, 0, 0, '2026-06-22 19:39:11');
INSERT INTO `it_ticket` VALUES (5, 'TK202606230001', '电脑无法开机', '就是电脑无法开机', 1, 'normal', 'cancelled', 1, NULL, NULL, NULL, '2026-06-23 00:29:35', NULL, NULL, NULL, NULL, NULL, NULL, '2026-06-23 01:29:35', '2026-06-23 08:29:35', NULL, NULL, 0, 0, '2026-06-23 00:52:15');
INSERT INTO `it_ticket` VALUES (6, 'TK202606230002', '打印机无法打印', '打印机无法打印', 3, 'normal', 'pending_accept', 2, NULL, NULL, NULL, '2026-06-23 01:42:31', NULL, NULL, NULL, NULL, NULL, NULL, '2026-06-23 02:42:31', '2026-06-23 09:42:31', NULL, NULL, 0, 0, '2026-06-23 01:42:31');
...
) ENGINE = InnoDB AUTO_INCREMENT = 151 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '系统权限表' ROW_FORMAT = Dynamic;
...
INSERT INTO `sys_permission` VALUES (146, 'ticket_category:list', '查看工单分类', '工单分类管理', 'api', 'GET', '/api/v1/ticket-categories', '查看工单分类列表、树和详情', 146, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `sys_permission` VALUES (147, 'ticket_category:create', '创建工单分类', '工单分类管理', 'api', 'POST', '/api/v1/ticket-categories', '创建工单分类', 147, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `sys_permission` VALUES (148, 'ticket_category:update', '修改工单分类', '工单分类管理', 'api', 'PUT', '/api/v1/ticket-categories/{category_id}', '修改工单分类', 148, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `sys_permission` VALUES (149, 'ticket_category:delete', '删除工单分类', '工单分类管理', 'api', 'DELETE', '/api/v1/ticket-categories/{category_id}', '删除或停用工单分类', 149, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
INSERT INTO `sys_permission` VALUES (150, 'ticket_category:status', '启用停用工单分类', '工单分类管理', 'api', 'PATCH', '/api/v1/ticket-categories/{category_id}/status', '启用或停用工单分类', 150, 1, '2026-06-26 00:00:00', '2026-06-26 00:00:00');
...
) ENGINE = InnoDB AUTO_INCREMENT = 127 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '角色权限关联表' ROW_FORMAT = Dynamic;
...
INSERT INTO `sys_role_permission` VALUES (122, 1, 146, '2026-06-26 00:00:00');
INSERT INTO `sys_role_permission` VALUES (123, 1, 147, '2026-06-26 00:00:00');
INSERT INTO `sys_role_permission` VALUES (124, 1, 148, '2026-06-26 00:00:00');
INSERT INTO `sys_role_permission` VALUES (125, 1, 149, '2026-06-26 00:00:00');
INSERT INTO `sys_role_permission` VALUES (126, 1, 150, '2026-06-26 00:00:00');
